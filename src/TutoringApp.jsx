...
// Progress for the small game apps lives on a roster doc per student. Those apps
// now write it under the student's uid, but their old name-keyed doc is still
// there frozen at the moment of migration -- reading only that one made the
// Report's auto-filled "Lesson completed" come from stale data (missing
// everything done since). This reads both and merges: lists are unioned, other
// fields prefer the uid doc. Same shape the callers already expect
// (.exists() / .data()).
const readAppRoster = async (appIdValue, studentUidValue, sanitizeFn, studentNameValue) => {
  const base = ['artifacts', appIdValue, 'public', 'data', 'roster'];
  const reads = [getDoc(doc(db, ...base, sanitizeFn(studentNameValue)))];
  if (studentUidValue && studentUidValue !== 'offline') reads.push(getDoc(doc(db, ...base, studentUidValue)));
  const snaps = await Promise.all(reads);
  const datas = snaps.filter(s => s.exists()).map(s => s.data());
  const merged = {};
  datas.forEach(d => Object.entries(d).forEach(([k, v]) => {
    if (Array.isArray(v) && Array.isArray(merged[k])) merged[k] = [...new Set([...merged[k], ...v])];
    else merged[k] = v;
  }));
  return { exists: () => datas.length > 0, data: () => merged };
};
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center flex-wrap gap-3 mb-6">
        ...
        {onOpenShrineRoom && (
          <button
            onClick={() => onOpenShrineRoom({ mode: 'teacher' })}
            className="flex items-center justify-center text-2xl bg-amber-50 hover:bg-amber-100 px-4 py-2 rounded-lg transition-colors border border-amber-200"
            title="Shrine Room (preview)"
          >
            🛕
          </button>
        )}
...
const getEffectivePreviousUnit = (lessonKey, sessionForCalc) => {
  ...
  // Only looks at the TOP-most lesson in Available Lessons (the one this
      ...
      <div className="py-6 mb-8 relative flex flex-col md:flex-row justify-between items-start md:items-center">
            <div className="w-full">
                <div className="flex items-center flex-wrap gap-3 mb-2">
                  ...
                  {onOpenShrineRoom && (
                    <button
                      onClick={() => onOpenShrineRoom({ studentUid, studentName: studentProfile?.name || '' })}
                      className="flex items-center justify-center text-2xl bg-amber-50 hover:bg-amber-100 w-11 h-11 rounded-lg transition-colors border border-amber-200 flex-shrink-0"
                      title="My Shrine Room"
                    >
                      🛕
                    </button>
                  )}
                  {onOpenAvatar && (
                    <button
                      onClick={() => onOpenAvatar({ studentUid, studentName: studentProfile?.name || '' })}
                  ...
                  {onOpenNatureWorld && (
                    <button
                      onClick={() => onOpenNatureWorld({ studentUid, studentName: studentProfile?.name || '' })}
                      className="flex items-center justify-center text-2xl bg-lime-50 hover:bg-lime-100 w-11 h-11 rounded-lg transition-colors border border-lime-200 flex-shrink-0"
                      title="My Nature World"
                    >
                      🌿
                    </button>
                  )}
                ...
                <div className="flex items-center flex-wrap gap-3 mb-2">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center text-gray-500 hover:text-red-700 bg-gray-50 hover:bg-red-50 w-11 h-11 rounded-lg transition-colors border border-gray-200 flex-shrink-0"
