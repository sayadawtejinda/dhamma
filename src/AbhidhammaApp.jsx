...
const abhiCompRef       = ()          => collection(db, P('quizCompletions'));
const abhiActivityRef   = ()          => collection(db, P('activity_feed'));
const abhiQRef          = (cId, lId)  => collection(db, P(`classes/${cId}/questions/${lId}/items`));
const abhiResultsRef    = (cId,lId,g) => collection(db, P(`classes/${cId}/quizResults/${lId}/${g}`));
const abhiPrefsRef      = (cId)       => doc(db, P(`classes/${cId}`));
...
const SmartContent = ({ text, imageBase }) => {
  ...
  return <span className="leading-relaxed">{parts.map((p,i)=>{
    ...
    if(isUrl){ if(p.match(/(youtube|youtu\.be)/i)) return <a key={i} href={p} target="_blank" rel="noreferrer" className="inline-flex items-center text-red-400 mx-1"><Youtube className="w-5 h-5 mr-1"/>Video</a>;
      if(isAu) return <AudioPlayer key={i} src={p}/>;
      ...
      return <a key={i} href={p} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline mx-1">Link<ExternalLink className="w-3 h-3 ml-1 inline"/></a>;
    }
...
const QuizModule = ({ classId, lessonId, lessonTitle, userId, userName, ageGroup, quizData, onClose }) => {
  ...
  const [qi,setQi]=useState(0); const [score,setScore]=useState(0); const [tl,setTl]=useState(30);
  const [show,setShow]=useState(false); const [sel,setSel]=useState(null); const [fb,setFb]=useState(null);
  const [proc,setProc]=useState(false); const [correct,setCorrect]=useState(0);
  ...
  const handleAnswer = async (idx, timeout=false) => {
    if(proc) return;
    setProc(true); setSel(idx);
    const q=quizData.questions[qi];
    const ok=!timeout&&q.correctAnswerIndex!==undefined&&idx===q.correctAnswerIndex;
    setFb(ok?'correct':'incorrect');
    const pts=ok?Math.max(100,Math.ceil((tl/30)*1000)):0;
    ...
    setTimeout(async()=>{
      ...
      if(qi+1<quizData.questions.length){ setQi(p=>p+1); setShow(false); setTl(30); }
        ...
        const fs=score+pts;
        if(nc<8){setState('failed');return;}
        setState('finished');
        try {
  ...
  if(state==='failed') return <div className="fixed inset-0 bg-gray-900 z-50 flex items-center justify-center p-4"><div className="bg-gray-800 p-8 rounded-2xl text-center border border-red-500"><div className="text-6xl mb-4">😔</div><h2 className="text-3xl font-bold text-red-400 mb-2">Not Enough Correct!</h2><p className="text-gray-300 mb-6">Need at least <span className="text-yellow-400 font-black">8 correct</span> to pass.</p><button onClick={onClose} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-full">Go Back & Review</button></div></div>;
    ...
    <div className="fixed inset-0 bg-gray-900 z-50 flex flex-col items-center justify-center p-4 overflow-y-auto">
      ...
      <div className="w-full max-w-3xl flex flex-col h-full justify-center">
        <div className="flex justify-between items-center mb-6 px-4">
          <div className="bg-gray-800 px-4 py-2 rounded-full border border-gray-600 text-white font-bold">Q{qi+1}/{quizData.questions.length}</div>
          {show&&<div className="bg-yellow-500 text-black px-6 py-2 rounded-full font-black text-xl flex items-center"><Timer className="w-5 h-5 mr-2"/>{tl}s</div>}
        ...
        <div className="bg-white text-black p-8 rounded-xl shadow-2xl mb-8 text-center min-h-[200px] flex items-center justify-center relative overflow-hidden">
          <h2 className="text-2xl md:text-3xl font-bold z-10">{quizData.questions[qi].question}</h2>
          {fb&&<div className={`absolute inset-0 flex items-center justify-center z-20 ${fb==='correct'?'bg-green-100/90 text-green-700':'bg-red-100/90 text-red-700'}`}><div className="text-4xl font-black">{fb==='correct'?'CORRECT!':'WRONG!'}</div></div>}
        </div>
        {show?(
          <div className={`grid gap-4 ${quizData.questions[qi].options.length===2?'grid-cols-2':'grid-cols-1 md:grid-cols-2'}`}>
            {quizData.questions[qi].options.map((opt,idx)=>{
              let cls="hover:scale-105 active:scale-95";
              let bg=['bg-red-500','bg-blue-500','bg-yellow-500','bg-green-500'][idx%4];
              if(fb){cls="cursor-not-allowed opacity-50";if(idx===quizData.questions[qi].correctAnswerIndex)bg="bg-green-600 ring-4 ring-green-300 opacity-100";else if(idx===sel&&fb==='incorrect')bg="bg-red-600 ring-4 ring-red-300 opacity-100";}
              return <button key={idx} onClick={()=>handleAnswer(idx)} disabled={proc} className={`${bg} text-white font-bold text-lg p-6 rounded-xl shadow-lg transform transition flex items-center ${cls}`}><span className="bg-black/20 w-8 h-8 rounded flex items-center justify-center mr-4">{['A','B','C','D'][idx]}</span>{opt}</button>;
            })}
          </div>
...
const NotificationBell = ({ userId }) => {
  ...
  const [lr,setLr]=useState(()=>parseInt(localStorage.getItem(`abhidhamma_notif_${userId}`))||0);
  useEffect(()=>{
    if(!db||!userId) return;
    const q=query(abhiActivityRef(),orderBy('timestamp','desc'),limit(15));
    return onSnapshot(q,snap=>setNotifs(snap.docs.map(d=>({id:d.id,...d.data()}))));
  },[userId]);
  const uc=notifs.filter(n=>{const ts=n.timestamp?.toMillis?n.timestamp.toMillis():(n.timestamp?.seconds*1000)||0;return ts>lr;}).length;
  const toggle=()=>{if(!open){const now=Date.now();setLr(now);localStorage.setItem(`abhidhamma_notif_${userId}`,now);}setOpen(!open);};
  return (
    <div className="relative">
      <button onClick={toggle} className="p-2 bg-gray-700 hover:bg-gray-600 rounded-full relative shadow-lg">
        <Bell className="w-5 h-5 text-gray-300"/>
        {uc>0&&<span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full animate-pulse">{uc>9?'9+':uc}</span>}
      </button>
      {open&&(
        <div className="absolute right-0 mt-2 w-72 max-w-[90vw] bg-gray-800 border border-gray-600 rounded-xl shadow-2xl z-50 overflow-hidden">
          <div className="p-3 border-b border-gray-700 flex justify-between items-center"><h4 className="font-bold text-white text-sm flex items-center gap-2"><Bell className="w-4 h-4 text-indigo-400"/>Notifications</h4><button onClick={()=>setOpen(false)} className="text-gray-400 hover:text-white"><X className="w-4 h-4"/></button></div>
          <div className="max-h-64 overflow-y-auto p-2 space-y-2">
            {notifs.length===0?<p className="text-center text-gray-500 text-xs py-6 italic">No new notifications</p>:notifs.map(n=>(
              <div key={n.id} className="bg-gray-700/50 p-3 rounded-lg border border-gray-600/50 flex items-start gap-3"><Trophy className="w-4 h-4 text-yellow-400 mt-1"/><div><p className="text-xs font-bold text-white"><span className="text-indigo-300">{n.studentName}</span> finished a quiz!</p><p className="text-[10px] text-gray-400 truncate mt-0.5">{n.lessonTitle}</p></div></div>
            ))}
          </div>
        </div>
      )}
...
// ─── AbhiClassRoster (per-class, SmartStudy-compatible) ───────────────────────
const AbhiClassRoster = ({ userId, classId }) => {
  const [students,setStudents]=useState([]); const [open,setOpen]=useState(true);
  const [now,setNow]=useState(Date.now()); const [aa,setAa]=useState(false);
  ...
  useEffect(()=>{
    ...
    return onSnapshot(q,snap=>{const n=Date.now();setStudents(snap.docs.map(d=>{const data=d.data(),lp=data.lastPing;if(data.isOnline&&lp){const pm=lp.toMillis?lp.toMillis():(lp.seconds*1000);if((n-pm)/60000>2)return{id:d.id,...data,isOnline:false};}return{id:d.id,...data};}));});
  },[classId]);
  useEffect(()=>{
  ...
  const approve=async(docId,name)=>{const ref=doc(db,P(`classRoster/${docId}`));const snap=await getDoc(ref);if(!snap.exists())return;const data=snap.data();let num=data.studentNumber;if(!num){const max=students.filter(s=>s.status==='approved').reduce((m,s)=>Math.max(m,s.studentNumber||0),0);num=max+1;}await updateDoc(ref,{status:'approved',name:name||data.name,pendingName:null,studentNumber:num});};
  ...
  const offline=students.filter(s=>s.status==='approved'&&!s.isOnline).sort((a,b)=>(a.studentNumber||0)-(b.studentNumber||0)).map(s=>{
    let w=false;const ct=s.lastSeen||s.lastPing;if(ct){const ms=ct.toMillis?ct.toMillis():(ct.seconds*1000);const d=(now-ms)/60000;if(d>=3&&d<=8)w=true;}return{...s,isWarning:w};
  });
  return (
    <div className="bg-gray-800 rounded-xl shadow-xl border border-gray-700 mb-6 overflow-hidden">
      <div onClick={()=>setOpen(!open)} className="p-4 border-b border-gray-700 cursor-pointer flex flex-wrap gap-3 justify-between items-center hover:bg-gray-700 transition">
        <div className="flex items-center gap-4">
          ...
          <button onClick={toggleAA} className={`flex items-center gap-1 text-xs px-3 py-1 rounded-full font-bold ${aa?'bg-green-500/20 text-green-400 border border-green-500/50':'bg-gray-800 text-gray-400 border border-gray-600'}`}>{aa?<ToggleRight className="w-4 h-4"/>:<ToggleLeft className="w-4 h-4"/>}Auto-Approve</button>
        </div>
          ...
          <div className="space-y-3">
            ...
            {pending.map(s=>(
              <div key={s.id} className="bg-gray-800 p-3 rounded-lg border border-yellow-600/30 flex justify-between items-center">
                ...
                <div className="flex gap-2"><button onClick={()=>approve(s.id,s.pendingName||s.name)} className="p-2 bg-green-600 rounded text-white"><UserCheck className="w-4 h-4"/></button><button onClick={()=>reject(s.id)} className="p-2 bg-red-600 rounded text-white"><UserX className="w-4 h-4"/></button></div>
              </div>
            ))}
          </div>
          <div className="space-y-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {online.length===0&&<span className="text-gray-600 text-sm italic">Nobody.</span>}
                {online.map(s=>(
                  <div key={s.id} className="bg-indigo-900/60 border border-indigo-500/50 px-3 py-1.5 rounded-xl flex items-center gap-2 text-sm">
                    ...
                    <button onClick={e=>remove(e,s.id)} className="text-gray-500 hover:text-red-400"><Trash2 className="w-3 h-3"/></button>
                  </div>
                ))}
              </div>
            </div>
...
const AbhiWelcomeModal = ({ userId, classId, onStudentComplete, onTeacherComplete }) => {
  const [step,setStep]=useState('role_select'); const [name,setName]=useState(''); const [grp,setGrp]=useState(null);
  const [pass,setPass]=useState(''); const [err,setErr]=useState(''); const [busy,setBusy]=useState(false);

  const handleSubmit = async () => {
    if(!name.trim()||!grp) return; setBusy(true);
    try {
      const rRef=abhiRosterDocRef(classId,name.trim()); const cRef=abhiClassDocRef(classId);
      const [rSnap,cSnap]=await Promise.all([getDoc(rRef),getDoc(cRef)]);
      const autoApprove=cSnap.exists()?cSnap.data().autoApprove:false;
      const status=autoApprove?'approved':'pending';
      if(rSnap.exists()){ const d=rSnap.data(); if(d.status==='approved'){onStudentComplete({...d});return;} await updateDoc(rRef,{pendingName:name.trim(),group:grp,status}); }
      else { await setDoc(rRef,{classId,studentName:name.trim(),name:name.trim(),group:grp,status,joinedAt:Date.now()}); }
      if(autoApprove){ const s=await getDoc(rRef); onStudentComplete({...s.data()}); } else setStep('waiting');
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 z-[60] flex items-center justify-center p-4">
      <div className="bg-amber-900/90 border border-amber-500 p-8 rounded-2xl shadow-2xl max-w-md w-full text-center relative overflow-hidden">
        ...
        {step==='role_select'&&<div className="animate-bounce-in">
        ...
        {step==='student'&&<div className="animate-bounce-in">
          <User className="w-16 h-16 mx-auto text-cyan-400 mb-4 animate-pulse"/>
          <h2 className="text-2xl font-black text-white mb-2">Student Setup</h2>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {Object.entries(AGE_GROUPS).map(([k,v])=>(
              <button key={k} onClick={()=>setGrp(k)} disabled={busy} className={`p-3 rounded-lg border flex flex-col items-center gap-2 transition ${grp===k?'bg-cyan-500 border-cyan-300 text-white scale-105':'bg-gray-800 border-gray-600 text-gray-400 hover:bg-gray-700'}`}>
                {v.icon}<span className="text-xs font-bold">{v.label}</span>
              </button>
            ))}
          </div>
          <button onClick={handleSubmit} disabled={!name||!grp||busy} className="w-full bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white font-black text-xl py-4 rounded-xl">
            {busy?<RotateCw className="w-6 h-6 mx-auto animate-spin"/>:'ENTER'}
          </button>
        </div>}
        {step==='waiting'&&<div className="animate-bounce-in"><Lock className="w-16 h-16 mx-auto text-yellow-400 mb-4 animate-pulse"/><h2 className="text-2xl font-black text-white mb-4">Waiting for Approval…</h2><RotateCw className="w-8 h-8 mx-auto text-cyan-400 animate-spin"/></div>}
        {step==='teacher'&&<div className="animate-bounce-in">
          ...
          <input type="password" className="w-full p-4 rounded-xl text-black font-bold text-center text-xl mb-4 outline-none" placeholder="Passcode" value={pass} onChange={e=>{setPass(e.target.value);setErr('');}} onKeyDown={e=>e.key==='Enter'&&(pass===TEACHER_PASSCODE?onTeacherComplete():setErr('Incorrect.'))}/>
          {err&&<p className="text-red-400 mb-4 text-sm font-bold">{err}</p>}
          <button onClick={()=>pass===TEACHER_PASSCODE?onTeacherComplete():setErr('Incorrect.')} className="w-full bg-purple-500 hover:bg-purple-600 text-white font-black text-xl py-4 rounded-xl">LOGIN</button>
        </div>}
      </div>
    </div>
  );
...
const AbhiLessonItem = ({ lesson, classId, isTeacher, studentAgeGroup, studentName, userId, onEdit, onGenerateVariants, onTakeQuiz, isGenerating, isOpen, onToggle }) => {
  const [tab,setTab]=useState('content'); const [hasAsked,setHasAsked]=useState(false); const [hasReplied,setHasReplied]=useState(false);
  const [isCompleted,setIsCompleted]=useState(false); const [leaderboard,setLb]=useState([]); const [showLb,setShowLb]=useState(false);
  ...
  useEffect(()=>{
    if(!classId||!userId||!lesson.id) return;
    const qRef=abhiQRef(classId,lesson.id);
    const u1=onSnapshot(query(qRef,orderBy('timestamp','asc')),snap=>{
      let asked=false,replied=false;
      snap.docs.forEach(d=>{const q=d.data();if(q.studentId===userId)asked=true;if(q.replies)q.replies.forEach(r=>{if(r.userId===userId)replied=true;});});
      setHasAsked(asked);setHasReplied(replied);
    });
    if(studentAgeGroup){
      const u2=onSnapshot(query(abhiResultsRef(classId,lesson.id,studentAgeGroup),where('userId','==',userId)),snap=>setIsCompleted(!snap.empty));
      return()=>{u1();u2();};
    }
    return()=>u1();
  ...
  useEffect(()=>{
    ...
    const scores={};
    getDocs(query(abhiScoresRef(),where('classId','==',classId),where('lessonId','==',lesson.id))).then(snap=>{
      snap.docs.forEach(d=>{const dt=d.data();if(!scores[dt.userId]||dt.score>scores[dt.userId].score)scores[dt.userId]=dt;});
      setLb(Object.values(scores).sort((a,b)=>b.score-a.score));
    });
  ...
  let dc='',dt=lesson.title,qa=false,qd=null,dq=[];
  if(!isTeacher&&studentAgeGroup){const v=variants[studentAgeGroup];if(v){dc=v.english;dt=v.englishTitle||lesson.title;qa=!!v.quiz;qd=v.quiz;dq=v.discussionQuestions||[];}else dc='Content not available for your age group yet.';}
  ...
  return (
    <div ref={ref} className={`relative rounded-xl shadow-md overflow-hidden border mb-4 ${isTeacher?'bg-gray-800 border-gray-700':'bg-gray-700 border-gray-600'}`}>
      {isGenerating&&<div className="absolute inset-0 bg-gray-900/80 z-10 flex flex-col items-center justify-center backdrop-blur-sm rounded-xl"><RotateCw className="w-12 h-12 text-teal-400 animate-spin mb-4"/><h3 className="text-xl font-bold text-white">Generating…</h3></div>}
      <div onClick={onToggle} className="p-4 cursor-pointer hover:bg-gray-600/50 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h3 className={`text-lg font-bold flex items-center gap-2 ${isTeacher?'text-teal-300':'text-white'}`}>
              {dt}{isCompleted&&<span className="bg-green-500/20 text-green-400 text-xs px-2 py-0.5 rounded-full flex items-center gap-1 border border-green-500/30"><CheckCheck className="w-3 h-3"/>Completed</span>}
            </h3>
        ...
        <div className="flex items-center gap-2">
          ...
          {isTeacher&&<div className="flex items-center gap-1 mr-2" onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={()=>onGenerateVariants(lesson,'junior')} className="px-3 py-1 bg-purple-600 rounded hover:bg-purple-700 text-white text-xs font-bold flex items-center gap-1"><Zap className="w-3 h-3"/>Jr.</button>
            <button onClick={()=>onGenerateVariants(lesson,'senior')} className="px-3 py-1 bg-pink-600 rounded hover:bg-pink-700 text-white text-xs font-bold flex items-center gap-1"><Zap className="w-3 h-3"/>Sr.</button>
          </div>}
        </div>
      </div>
            ...
            {qa&&(isQuizLocked&&!isCompleted?(
              <div className="bg-gray-800/80 p-4 rounded-xl border border-gray-600 flex flex-col items-center text-center">
                ...
                <div className="flex gap-4 text-xs font-semibold">
                  <span className={`flex items-center gap-1 ${hasAsked?'text-green-400':'text-gray-500'}`}>{hasAsked?<CheckCircle className="w-4 h-4"/>:<div className="w-4 h-4 rounded-full border border-gray-500"/>}Ask 1 Question</span>
                  <span className={`flex items-center gap-1 ${hasReplied?'text-green-400':'text-gray-500'}`}>{hasReplied?<CheckCircle className="w-4 h-4"/>:<div className="w-4 h-4 rounded-full border border-gray-500"/>}Answer 1 Question</span>
                </div>
              </div>
            ):(
              <div>
                <button onClick={e=>{e.stopPropagation();onTakeQuiz(lesson.id,dt,qd);}} className={`w-full py-3 font-black rounded-xl shadow-lg transform transition hover:scale-[1.02] flex items-center justify-center gap-2 ${isCompleted?'bg-gradient-to-r from-green-600 to-teal-600 text-white':'bg-gradient-to-r from-purple-600 to-indigo-600 text-white'}`}>
      ...
      {showLb&&(
        <div className="fixed inset-0 bg-gray-900/95 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-800 w-full max-w-lg rounded-2xl shadow-2xl p-6 border border-gray-700 relative">
            <button onClick={()=>setShowLb(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white"><X className="w-6 h-6"/></button>
            <div className="text-center mb-6"><Trophy className="w-16 h-16 text-yellow-400 mx-auto mb-2"/><h3 className="text-2xl font-black text-white">{dt}</h3></div>
            <div className="space-y-2 max-h-[60vh] overflow-y-auto">
              {leaderboard.length===0?<p className="text-center text-gray-500">No scores yet.</p>:leaderboard.map((e,idx)=>(
                <div key={idx} className={`flex justify-between items-center p-3 rounded ${e.userId===userId?'bg-indigo-600 border border-indigo-400':'bg-gray-700'}`}>
                  <div className="flex items-center gap-3"><span className="font-bold w-6 text-yellow-400">#{idx+1}</span><span className="font-semibold text-white">{e.name}{e.userId===userId&&<span className="text-[10px] bg-white text-indigo-600 px-1.5 py-0.5 rounded-full font-bold ml-2">ME</span>}</span></div>
                  <span className="font-mono font-bold text-indigo-300">{e.score} pts</span>
                </div>
              ))}
            </div>
          </div>
...
export default function AbhidhammaApp({ entryRequest, onExit }) {
  ...
  const [isTeacher,setIsTeacher]=useState(false); const [role,setRole]=useState('Student');
  const [classId,setClassId]=useState(''); const [classData,setClassData]=useState(null); const [lessons,setLessons]=useState([]);
  ...
  useEffect(()=>{
    ...
    return onSnapshot(abhiClassDocRef(classId),snap=>{if(snap.exists()){setClassData(snap.data());setLessons(snap.data().lessons||[]);}else{setClassData(null);setLessons([]);}});
  },[classId,authReady]);
  ...
  const createClass = async () => {
    ...
    await setDoc(abhiClassDocRef(newClassId.trim()),{classId:newClassId.trim(),lessons:[],autoApprove:false,createdAt:serverTimestamp()},{merge:true});
  ...
  const handleSaveLesson = async e => {
    ...
    setLoading(true);
    try {
      const entry={id:editingLesson?editingLesson.id:`lesson_${Date.now()}`,title:newTitle.trim(),burmeseContent:newContent.trim(),imageBaseUrl:newImgBase.trim()||DEFAULT_IMG_BASE,variants:editingLesson?.variants||{},createdAt:editingLesson?.createdAt||Date.now()};
      const cRef=abhiClassDocRef(classId);
      if(editingLesson){await updateDoc(cRef,{lessons:lessons.map(l=>l.id===editingLesson.id?entry:l)});}
      else{await updateDoc(cRef,{lessons:arrayUnion(entry)});}
  ...
  const handleGenerateVariants = async (lesson, mode) => {
    ...
    try {
      ...
      await updateDoc(abhiClassDocRef(classId),{lessons:lessons.map(l=>l.id===lesson.id?{...l,variants:{...l.variants,...nv}}:l)});
      showMsg('Generated!');
    }catch(e){showMsg('Error.');console.error(e);} finally{setGenId(null);}
  };

  // ── Export Lessons Only ──
  const handleExportLessonsOnly = () => {
  ...
  const handleExportFull = async () => {
    if(!classId){showMsg('Select a class first.');return;} setLoading(true); showMsg('Exporting full backup…');
    try {
      ...
      const [rosterSnap,scoresSnap,actSnap]=await Promise.all([
      ...
      const data={version:3,classId,timestamp:new Date().toISOString(),lessons,
      ...
      for(const l of lessons){
        for(const g of Object.keys(AGE_GROUPS)){
          const rSnap=await getDocs(abhiResultsRef(classId,l.id,g));
          if(!rSnap.empty) data.quizResults[`${l.id}_${g}`]=rSnap.docs.map(r=>({id:r.id,...r.data(),timestamp:toMs(r.data().timestamp)}));
        }
      }
      const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
      const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`abhidhamma-full-${classId}-${Date.now()}.json`;
      document.body.appendChild(a);a.click();document.body.removeChild(a);
      showMsg('Full backup exported!');
    }catch(e){console.error(e);showMsg('Error exporting!');} finally{setLoading(false);}
  };

  // ── Import Lessons Only (does NOT touch student records) ──
  const handleImportLessonsOnly = (event) => {
    ...
    reader.onload=async e=>{
      try {
        ...
        const newLessons=raw.map((l,i)=>({id:l.id||`imported_${Date.now()}_${i}`,title:l.title||`Lesson ${i+1}`,burmeseContent:l.burmeseContent||l.content||'',imageBaseUrl:l.imageBaseUrl||imgBase,variants:l.variants||{},createdAt:l.timestamp||Date.now()}));
        const cRef=abhiClassDocRef(tgt); const cSnap=await getDoc(cRef);
        const existing=cSnap.exists()?(cSnap.data().lessons||[]):[];
        const existIds=new Set(existing.map(l=>l.id));
        const toAdd=newLessons.filter(l=>!existIds.has(l.id));
        // LESSONS ONLY — only update lessons array, nothing else
        await setDoc(cRef,{classId:tgt,lessons:[...existing,...toAdd]},{merge:true});
        showMsg(`✅ Imported ${toAdd.length} lessons into "${tgt}". Student records untouched.`);
  ...
  // ── Import Full Backup (restores everything for the class) ──
  const handleImportFull = (event) => {
    ...
    reader.onload=async e=>{
      try {
        ...
        const newLessons=raw.map((l,i)=>({id:l.id||`imported_${Date.now()}_${i}`,title:l.title||`Lesson ${i+1}`,burmeseContent:l.burmeseContent||l.content||'',imageBaseUrl:l.imageBaseUrl||imgBase,variants:l.variants||{},createdAt:l.timestamp||Date.now()}));
        // Restore lessons
        const cRef=abhiClassDocRef(tgt);
        await setDoc(cRef,{classId:tgt,lessons:newLessons},{merge:true});
        // Restore roster
        for(const stu of (data.roster||[])){
          const rRef=abhiRosterDocRef(tgt,stu.studentName||stu.name||'');
          await setDoc(rRef,{...stu,classId:tgt},{merge:true});
        }
        // Restore scores
        for(const s of (data.scores||[])){const{id,timestamp,...r}=s;await setDoc(doc(abhiScoresRef(),id||`s_${Date.now()}`),{...r,classId:tgt,timestamp:toDate(timestamp)});}
        // Restore activity
        for(const a of (data.activityFeed||[])){const{id,timestamp,...r}=a;await setDoc(doc(abhiActivityRef(),id||`a_${Date.now()}`),{...r,timestamp:toDate(timestamp)});}
        // Restore quiz results
        for(const[key,rList]of Object.entries(data.quizResults||{})){
          const parts=key.split('_'); const g=parts.pop(); const lId=parts.join('_');
          for(const r of rList){const{id,timestamp,...rest}=r;await setDoc(doc(abhiResultsRef(tgt,lId,g),id||`r_${Date.now()}`),{...rest,timestamp:toDate(timestamp)});}
        }
        showMsg(`✅ Full restore: ${newLessons.length} lessons + student records into "${tgt}".`);
        if(!classId) enterClass(tgt);
      }catch(err){showMsg(`Error: ${err.message}`);}
      finally{setLoading(false);event.target.value='';}
    };
    ...
    <div className="min-h-screen bg-gray-900 text-white font-sans p-4 sm:p-8 pt-16">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="mb-6 flex flex-wrap gap-4 justify-between items-center bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-700">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isTeacher?(
              <div className="flex gap-2">
                ...
                <button onClick={()=>{setIsTeacher(false);setRole('Student');localStorage.removeItem('abhidhamma_isTeacher');}} className="px-4 py-2 font-bold rounded bg-red-600 hover:bg-red-700">Logout</button>
              </div>
            ):(
          ...
          <div className="flex items-center gap-3">
            <div className="text-gray-400 text-sm font-semibold">{role} View {classId&&`· ${classId}`}</div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-4">
              ...
              <div className="flex gap-2 mb-3">
                <input value={newClassId} onChange={e=>setNewClassId(e.target.value)} placeholder="New Class ID" className="flex-1 bg-gray-900 text-white p-2 rounded border border-gray-600 focus:border-amber-500 focus:outline-none text-sm" onKeyDown={e=>e.key==='Enter'&&createClass()}/>
                <button onClick={createClass} disabled={!newClassId.trim()} className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded font-bold text-sm disabled:opacity-50 flex items-center gap-1"><Plus className="w-4 h-4"/>Create</button>
              </div>
              <div className="flex flex-wrap gap-2">
                {allClasses.map(c=>(
                  <button key={c.id} onClick={()=>enterClass(c.id)} className={`px-3 py-1.5 rounded-lg text-sm font-semibold border transition-all ${c.id===classId?'bg-amber-500 text-white border-amber-400':'bg-gray-700 text-gray-300 border-gray-600 hover:border-amber-400'}`}>
                    {c.id} <span className="text-xs opacity-70">({(c.lessons||[]).length})</span>
                  </button>
                ))}
                {allClasses.length===0&&<p className="text-gray-500 text-sm italic">No classes yet.</p>}
              </div>
            </div>
              ...
              <div className="bg-gray-800 p-6 rounded-xl shadow-xl border border-gray-700">
                ...
                <div className="flex flex-wrap gap-2 mb-4 p-3 bg-gray-900 rounded-lg border border-gray-600">
                  <div className="flex items-center gap-2 flex-1 min-w-[200px]">
                    <span className="text-gray-400 text-xs whitespace-nowrap">Class ID for import:</span>
                    <input value={importClassId} onChange={e=>setImportClassId(e.target.value)} className="flex-1 bg-transparent text-white text-xs focus:outline-none border-b border-gray-600 px-1 focus:border-amber-400"/>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    <button onClick={handleExportLessonsOnly} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1"><Download className="w-3 h-3"/>📚 Lessons</button>
                ...
                <div className="flex items-center gap-2 mb-4 p-2 bg-gray-900 rounded border border-gray-600">
                  <span className="text-gray-400 text-xs ml-1 whitespace-nowrap">🖼 Image Folder URL:</span>
                  <input value={newImgBase} onChange={e=>setNewImgBase(e.target.value)} placeholder={DEFAULT_IMG_BASE} className="flex-1 bg-transparent text-white text-xs focus:outline-none border-b border-gray-600 px-1 focus:border-teal-500"/>
                </div>
                ...
                <form onSubmit={handleSaveLesson} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <button type="submit" disabled={loading} className="flex-1 bg-teal-600 p-3 rounded hover:bg-teal-700 flex justify-center items-center font-bold">
                      {loading?<RotateCw className="animate-spin w-5 h-5 mr-2"/>:<BookOpen className="w-5 h-5 mr-2"/>}{editingLesson?'Update':'Save Lesson'}
                    </button>
            ...
            {classId&&(
              <div className="space-y-4">
                {lessons.length===0&&<p className="text-center text-gray-500 py-6">No lessons in <strong>{classId}</strong> yet.</p>}
                {lessons.map(l=>(
                  <AbhiLessonItem key={l.id} lesson={l} classId={classId} isTeacher userId={userId}
                    onEdit={lesson=>{setEditingLesson(lesson);setNewTitle(lesson.title);setNewContent(lesson.burmeseContent);setNewImgBase(lesson.imageBaseUrl||DEFAULT_IMG_BASE);window.scrollTo({top:0,behavior:'smooth'});}}
                    onGenerateVariants={handleGenerateVariants} onTakeQuiz={()=>{}} isGenerating={genId===l.id}
          ...
          <div>
            {!classId&&(
              <div className="p-6 max-w-lg mx-auto mt-10">
                ...
                :allClasses.length===0?<p className="text-center text-gray-500 italic">No classes found yet.</p>
                :<div className="space-y-3">{allClasses.map(c=>(
                  <button key={c.id} onClick={()=>{if(!studentProfile){setClassId(c.id);setShowWelcome(true);}else enterClass(c.id);}}
                    className={`w-full p-4 rounded-xl border-2 text-left font-bold text-lg transition-all flex items-center justify-between ${c.id===entryRequest?.classId?'bg-amber-100 border-amber-500 text-amber-800 shadow-lg scale-[1.02]':'bg-white border-gray-200 text-gray-700 hover:border-amber-300'}`}>
                    <span>{c.id===entryRequest?.classId?'⭐ ':''}{c.id}</span>
                    <span className="text-sm font-normal text-gray-400">{(c.lessons||[]).length} lessons</span>
                  </button>
                ))}</div>}
              </div>
            )}
            {classId&&!studentProfile&&!showWelcome&&(
              <div className="text-center mt-10"><button onClick={()=>setShowWelcome(true)} className="bg-amber-600 hover:bg-amber-700 text-white font-bold py-4 px-8 rounded-xl text-xl">Enter Class</button></div>
            )}
            {classId&&studentProfile&&(
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={()=>{setClassId('');setClassData(null);setLessons([]);}} className="text-sm text-gray-400 hover:text-white underline">← Change Class</button>
                </div>
