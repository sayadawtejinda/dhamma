...
export default function AnimalSoundApp({ entryRequest, onExit, hideOwnOnlineBadge }) {
  ...
  useEffect(() => {
    ...
    const byId = (id) => rootEl.querySelector('#' + id);
        ...
        // Only the first 5 correct answers ever count toward "completed" /
        ...
        function recordWin() {
            if (trophyWins >= MAX_TROPHY_WINS) return;
            trophyWins++;
            if (progressRosterRef) {
                setDoc(progressRosterRef, { trophyWins }, { merge: true }).catch(() => {});
            }
        }
        ...
        window.__asApp = { dismissError, depositCoinsToShrineRoom };
  ...
  return (
    <>
      ...
      {!hideOwnOnlineBadge && (
        <OnlineStatusWidget
          rosterPath={AS_ROSTER_PATH}
          studentName={studentName}
          isTeacherMode={!studentName}
          coinBalance={studentName ? myCoinBalance : null}
          coinIcon="🪙"
          onCoinClick={studentName ? () => window.__asApp?.depositCoinsToShrineRoom?.() : undefined}
          panelTitle="🐾 Students"
          renderActivity={s => (
            <span className="text-gray-600">
              {s.trophyWins != null ? `${s.trophyWins}/5 wins` : 'Not practicing'}
