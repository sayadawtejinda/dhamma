...
export default function MyanmarSpellingApp({ entryRequest, onExit, hideOwnOnlineBadge }) {
  ...
  // This app has no game/scoring moment of its own to earn coins from (it's
  // a free-browse syllable reader) -- coinBalance is read-only display for
  // now (0 unless something else ever credits this roster doc), just so the
  // student/teacher header is consistent with every other app in the suite.
  ...
  // carries their name); a teacher just observes. Also loads this
  // student's coinBalance (0 unless something else has ever credited it)
  // for the header's coin badge.
  useEffect(() => {
    if (!studentName || !studentUid) return;
    const rosterRef = rosterDocRefByUid(db, MS_ROSTER_PATH, studentUid);
    migrateNameKeyedRosterDoc(db, MS_ROSTER_PATH, studentUid, studentName, sanitizeMsKey)
    ...
    }).catch(e => console.error('Error loading Myanmar Spelling coin balance:', e));
  ...
  // Deposits this student's entire local coin balance into their Shrine
  // Room wallet, same pattern as every other app that has a coin badge.
  async function depositCoinsToShrineRoom() {
    ...
    try {
      ...
      setMyCoinBalance(0);
      setDoc(rosterDocRefByUid(db, MS_ROSTER_PATH, studentUid), { coinBalance: 0 }, { merge: true }).catch(() => {});
  ...
  return (
    <>
      ...
      {!hideOwnOnlineBadge && (
        <OnlineStatusWidget
          rosterPath={MS_ROSTER_PATH}
          studentName={studentName}
          isTeacherMode={!studentName}
          panelTitle="🔤 Students"
          coinBalance={studentName ? myCoinBalance : null}
          onCoinClick={studentName ? depositCoinsToShrineRoom : undefined}
          renderActivity={s => s.coinBalance != null && (
            <span className="font-bold text-amber-600">🪙{s.coinBalance}</span>
          )}
        />
      )}
    </>
