...
export default function MyanmarVowelsLearningApp({ entryRequest, onExit, hideOwnOnlineBadge }) {
  ...
  useEffect(() => {
    ...
    const byId = (id) => rootEl.querySelector('#' + id);
        ...
        // completedGames holds ids like "B-listen-1", "P-click-1" -- winning
        ...
        function gainPoint() {
            // Typing Practice is excluded from coins/completion tracking --
            // it isn't taught yet, so students aren't expected to have
            // trophies for it (see recordGameCompleted in gameWin below).
            if (currentGameType !== 'type') awardCoins(10);
            currentScore++;
            updateProgressUI();
        ...
        function handleWrongAnswer(wrongAnswerDisplay) {
            if (currentGameType !== 'type') awardCoins(-1);
            let dropped = false;
            if (currentScore > 0) {
                currentScore--;
                dropped = true;
        ...
        function gameWin() {
            if (currentGameType !== 'type') {
                recordGameCompleted(`${currentMode}-${currentGameType}-${currentGameLevel}`);
            }
            clearInterval(gameLoopInterval);
  ...
  return (
    <>
      ...
      {!hideOwnOnlineBadge && (
        <OnlineStatusWidget
          rosterPath={MVL_ROSTER_PATH}
          studentName={studentName}
          isTeacherMode={!studentName}
          coinBalance={studentName ? myCoinBalance : null}
          coinIcon="🪙"
          onCoinClick={studentName ? () => window.__mvlApp?.depositCoinsToShrineRoom?.() : undefined}
          panelTitle="🔤 Students"
          renderActivity={s => (
            <span className="text-gray-600">
              {s.completedGames?.length ? `${s.completedGames.length} games done` : 'Not practicing'}
